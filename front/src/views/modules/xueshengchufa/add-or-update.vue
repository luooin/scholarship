    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="处罚原因" prop="chufayuanyin">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.chufayuanyin" label="处罚原因" prop="chufayuanyin">
                    <span v-html="ruleForm.chufayuanyin"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.chufaneirong = this.ruleForm.chufaneirong.replace(reg,'../../../scholarship/upload');
	this.ruleForm.chufayuanyin = this.ruleForm.chufayuanyin.replace(reg,'../../../scholarship/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshengchufa/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshengchufaCrossAddOrUpdateFlag = false;
						 this.parent.search();
